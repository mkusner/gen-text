...
X = np.loadtxt('../../../bo_optimization_character/latent_features_and_targets/latent_faetures.txt')
y = -np.loadtxt('../../../bo_optimization_character/latent_features_and_targets/targets.txt')
y = y.reshape((-1, 1))
...
while iteration < 1000:
    ...
    if iteration == 0 and os.path.isfile("results_gp_multiple_iterations/X_train.dat") and \
        os.path.isfile("results_gp_multiple_iterations/y_train.dat") and os.path.isfile("results_gp_multiple_iterations/iteration.dat"):

        X_train = load_object("results_gp_multiple_iterations/X_train.dat")
        y_train = load_object("results_gp_multiple_iterations/y_train.dat")
        iteration = load_object("results_gp_multiple_iterations/iteration.dat")
    else:
        save_object(X_train, "results_gp_multiple_iterations/X_train.dat")
        save_object(y_train, "results_gp_multiple_iterations/y_train.dat")
        save_object(iteration, "results_gp_multiple_iterations/iteration.dat")
    ...
    sys.path.insert(0, '../../../')
    import molecule_vae
    grammar_weights = '../../../weight_files/zinc_str_vae_L56_E99.hdf5'
...
#            valid_smiles_final_final.append(valid_smiles_final[ i ])
...
#    valid_smiles_final = valid_smiles_final_final
    ...
    save_object(valid_smiles_final, "results_gp_multiple_iterations/valid_smiles{}.dat".format(iteration))

    logP_values = np.loadtxt('../../../bo_optimization_character/latent_features_and_targets/logP_values.txt')
    SA_scores = np.loadtxt('../../../bo_optimization_character/latent_features_and_targets/SA_scores.txt')
    cycle_scores = np.loadtxt('../../../bo_optimization_character/latent_features_and_targets/cycle_scores.txt')
